...
if version_info.major != 3 or version_info.minor < 10:
    print("python version 3.10 or newer is required", file=stderr)
    _exit(1)
...
def allow_functions():
    ...
    basics = {
        ...
        "__init_subclass__",
        "__del__",
        "__getstate__",  # new in cpython 3.11
        "__module__",  # new in cpython 3.12
    ...
    allow[set] = basics.copy()
    allow[deque] = basics.copy()

    allow[d_keys] = basics | {"__iter__"}
    allow[d_items] = basics | {"__iter__"}
    allow[d_values] = basics.copy()

    allow[generator] = basics.copy()
    allow[coroutine] = basics.copy()
    allow[builtins] = {
        "min",
        "max",
        "abs",
        ...
        "str",
        "int",
        "bool",  # killed by overrides_late()
    }

    if level >= 3:
        allow[int] |= {"__divmod__"}
        allow[builtins] |= {"divmod", "list"}  # list is neutered by its wrapper

    if level >= 3 and allow_data_structures:
        allow[list] |= {"__len__", "__setitem__", "append", "pop", "copy"}
        allow[builtins] |= {"len", "sum", "enumerate"}

    if level >= 5 and allow_data_structures:
        allow[set] |= {"__contains__", "__iter__", "__len__", "copy", "add", "remove"}
    ...
    if level >= 6 and allow_data_structures:
        allow[set] |= {
            "__or__",
            "__and__",
            "__sub__",
    ...
    if level >= 11:
        allow[builtins] |= {"open", "bytes", "repr"}  # FIXME repr
        allow[str] |= {
            "__getitem__",
            "__iter__",
            "__len__",
...
def allow_modules():
    ...
    if level >= 11:
        allow["gzip"] = ["open"]
        allow["sys"] = ["argv", "stdin", "stdout", "stderr"]
        allow["csv"] = ["reader", "writer", "DictReader"]
        allow["os"] = ["path", "remove"]
...
import os
import ctypes
from sys import settrace, gettrace, modules, exit, flags, exc_info, getrecursionlimit
...
c_init_f = ctypes.CFUNCTYPE(c_int, c_pyobject, c_pyobject, c_void_p)
...
tab_pyobject = [
    ...
    ("init", "tp_init", c_init_f),
    (None, "tp_alloc", c_void_p),
    ("new", "tp_new", c_void_p),
    (None, "tp_free", c_void_p),
    (None, "tp_is_gc", c_void_p),
    (None, "tp_bases", c_void_p),
    (None, "tp_mro", c_void_p),
    (None, "tp_cache", c_void_p),
    (None, "tp_subclasses", c_void_p),
    (None, "tp_weaklist", c_void_p),
    (None, "tp_del", c_void_p),
    (None, "tp_version_tag", c_int),
    (None, "tp_finalize", c_void_p),
    ("vcall", "tp_vectorcall", c_void_p),
]
    ...
    if _id(klass) == _id(builtins):
        ...
        for patch_ in _gen_iter(_dict_getitem(dunders, pyattr)(cklass)):
            ...
            if patch_obj.__class__ is struct_sequence:
                if (
                    klass is _tuple
                    and attr != "__contains__"
                    or attr == "__subscript__"
                ):
                    continue
...
def error_print(trace, *args):
    os.write(2, b"traceback:\n")
    for frame in _reversed(trace):
        os.write(
            2,
            bytes(
    ...
    os.write(2, bytes("error: " + "".join(map(str, args)) + "\n", "utf8"))
...
def wrap_open(attr, old, *args, **kwargs):
    with overrides_disabled():
        fix_open_kwargs(args, kwargs)
        return old(*args, **kwargs)


def fix_open_kwargs(args, kwargs):
...
def import_hacks(old, name, allow, *args, **kwargs):
    if name == "io" and not allow:
...
def without_overrides(func, is_open=False):
    ...
    return wrap_open if is_open else wrap


def wrap_attr(klass, attr, is_open=False):
    _setattr(klass, attr, without_overrides(_getattr(klass, attr), is_open))
...
def make_proxy(mod_name, mod, allow):
    ...
    if mod_name == "gzip":
        ...
        wrap_attr(mod, "open", is_open=True)
    if mod_name == "os":
    ...
    if mod_name == "csv":
        ...
        mod.zip = _zip
    if mod_name == "random":
        wrap_attr(mod.Random, "seed")
        wrap_attr(mod.Random, "randint")
        wrap_attr(mod.Random, "random")
        wrap_attr(mod.Random, "shuffle")
        wrap_attr(mod.Random, "choice")
        wrap_attr(mod.Random, "sample")
...
def wrap_import(attr, old, name, *args, **kwargs):
    with overrides_disabled():
        ...
        hack = import_hacks(old, name, allow, *args, **kwargs)
...
def wrap_type_or(attr, old, self, other):
    if self is not None and other is not None:
        _error("union types other than x | None are disabled")
    if self is None:
        self = _type(None)
    return old(self, other)


def wrap_enumerate(attr, old, arg):
    if _type(arg) != list:
        _error("enumerate can only be used on lists")
    return old(arg)
...
def wrap_truediv(attr, old, self, arg):
    if not _isinstance(arg, _float):
        _error("at least one argument of / must be a float")
    return old(self, arg)


def wrap_list_init(attr, old, self, args, kwargs):
    if _tuple_len(args) == 1:
        if _isinstance(_tuple_getitem(args, 0), _tuple):
            _error("construction of lists from tuples is disabled")
        if _isinstance(_tuple_getitem(args, 0), _str) and level < 11:
            _error("construction of lists from strings is disabled")
    if not allow_data_structures:
        _error("constructing lists is not allowed")
    old(self, *args)
    return 0  # = success
...
class refuse_str(str):
    def __init__(self, *args):
        _error("the built-in function str() is disabled")


class refuse_int(int):
    def __init__(self, *args):
        _error("the built-in function int() is disabled")


class refuse_bool(int):
    def __init__(self, *args):
        _error("the built-in function bool() is disabled")


def wrap_float_init(attr, old, self, args, kwargs):
    if _tuple_len(args) != 1 or not _isinstance(_tuple_getitem(args, 0), _int):
        _error("the built-in function float() is disabled except for float(int)")
    return 0


def wrap_isinstance(attr, old, value, type_):
    if _isinstance(value, _int) and type_ is refuse_int:
        return True
    if _isinstance(value, _float) and type_ is wrap_float:
        return True
    if _isinstance(value, _bool) and type_ is refuse_bool:
        return True
    if _isinstance(value, _str) and type_ is refuse_str:
        return True
    if _isinstance(value, _tuple) and type_ is refuse_tuple:
        return True

    return _isinstance(value, type_)
...
def overrides_late():
    ...
    override(builtins, "str", refuse_str)
    override(builtins, "int", refuse_int)
    override(builtins, "bool", refuse_bool)
    override(_float, "__init__", wrap_float_init)

    if is_allowed(builtins, "enumerate"):
        override(builtins, "enumerate", wrap_enumerate)

    if is_allowed(builtins, "isinstance"):
        override(builtins, "isinstance", wrap_isinstance)
    else:
        override(_type, "__or__", wrap_type_or)
    ...
    if level >= 3:
        ...
        override(_list, "__init__", wrap_list_init)

    # disable tp_vectorcall so that list.__init__ actually gets called
    patch(_list, "__vcall__", 0)
    patch(_float, "__vcall__", 0)
    patch_clear_cache()
...
def overrides_early():
    ...
    if is_allowed(builtins, "open"):
        override(builtins, "open", wrap_open)
...
def freeze_globals():
    ...
    with overrides_disabled():
        ...
        for name, obj in gdict.items():
            ...
            if type(obj) in [set, dict, list, tuple, str, int, float, bool]:
                frozen[name] = deepcopy(obj)
            elif type(obj) is GenericAlias or type(obj) is UnionType:
                pass
            else:
...
def restrict_trace(frame, why, arg):
    ...
    global cfunc_exception, recursion_depth
    ...
    if why == "call":
        recursion_depth += 1
        if recursion_depth > getrecursionlimit() - 50:
            _error("recursion limit exceeded at depth ", recursion_depth)
...
def check_import():
    ...
    global level, level_str, script, code, gdict, allow_data_structures
    ...
    matched = fullmatch(
        f"from {__name__} import week_(..)" "(, except_data_structures|)(  # noqa|)\n",
        line,
    )
...
def check_ast(node, in_def=False, in_class=False, in_assign=False):
    ...
    if node_is(ast.Dict) and level < 5:
        bad("dict is disabled")
    if node_is(ast.Set) and level < 5:
        bad("set is disabled")
    ...
    if node_is(ast.With) and level < 11:
